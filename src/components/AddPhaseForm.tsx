...
export default function AddPhaseForm({ projectId, nextOrder }: { projectId: string; nextOrder: number }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      className="inline-flex items-center gap-1.5 text-sm text-[#22C55E] hover:text-[#16a34a] font-semibold transition-colors py-1">
      <Plus size={15} /> Add Phase
    </button>
  )

  return (
    <div className="flex gap-2 items-center bg-white rounded-2xl border border-[#22C55E]/30 px-3 py-2">
      ...
      <button onClick={handleAdd} disabled={loading || !name.trim()}
        className="bg-[#22C55E] hover:bg-[#16a34a] text-white px-3 py-1.5 rounded-lg text-xs font-bold disabled:opacity-50 transition-colors">
        Add
      </button>
