...
function NewProjectForm() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {templateName && (
        <div className="mb-5 bg-[#6366F1]/10 border border-[#6366F1]/30 rounded-xl px-4 py-3 flex items-center gap-2">
          <LayoutTemplate size={15} className="text-[#6366F1]" />
          <p className="text-sm text-[#4f46e5] font-medium">Using template: <strong>{templateName}</strong></p>
          <Link href="/projects/new" className="ml-auto text-xs text-[#4f46e5] hover:underline">Remove</Link>
        </div>
      )}

      {!templateId && (
        <div className="mb-5">
          <Link href="/templates" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-[#6366F1] transition-colors">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-7 space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading}
            className="flex-1 bg-[#6366F1] hover:bg-[#4f46e5] text-white font-bold py-3 px-4 rounded-xl transition-all disabled:opacity-50 shadow-sm">
