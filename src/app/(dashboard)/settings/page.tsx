      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6">
          ...
          <div className="flex items-center gap-4 mb-5">
            <div className="w-14 h-14 rounded-2xl bg-[#0F172A] flex items-center justify-center text-white text-xl font-bold flex-shrink-0">
              {(profile?.full_name ?? user?.email ?? '?')[0]?.toUpperCase()}
            </div>
            <div>
              <p className="font-semibold text-gray-900">{profile?.full_name ?? 'No name set'}</p>
              <p className="text-sm text-gray-400">{user?.email}</p>
              <span className="inline-block mt-1 text-xs font-semibold bg-[#0F172A]/5 text-[#0F172A] px-2 py-0.5 rounded-full capitalize">
                {profile?.role ?? 'client'}
              </span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6">
          ...
          <div className="space-y-2 text-sm text-gray-600">
            ...
            <div className="flex justify-between py-2">
              ...
              <a href="mailto:ask.dot.it@gmail.com" className="font-medium text-[#6366F1] hover:underline">ask.dot.it@gmail.com</a>
            </div>
          </div>
        </div>
      </div>
